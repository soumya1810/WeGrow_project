...
class Rating{
	...
	public function getdescshopList(){
		$sqlQuery = "
			SELECT * FROM ".$this->shopTable."
			ORDER BY id DESC ";
		return  $this->getData($sqlQuery);	
	}
	public function getcatshopList($category_name){
		$sqlQuery = "
			SELECT * FROM ".$this->shopTable."
			WHERE domain='".$category_name."'";
		return  $this->getData($sqlQuery);	
	}
	public function getstateshopList($state_name){
		$sqlQuery = "
			SELECT * FROM ".$this->shopTable."
			WHERE state='".$state_name."'";
		return  $this->getData($sqlQuery);	
	}
	public function getcatstateshopList($category_name,$state_name){
		$sqlQuery = "
			SELECT * FROM ".$this->shopTable."
			WHERE domain='".$category_name."' 
